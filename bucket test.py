...
def main():
    ...
    radius= 300
    angle_const=100
    angle= randint(-angle_const, angle_const)
    count = angle
    win = tk.Tk()
...
class user_id():
    ...
    def print_results(self):
        ...
        name_of_file=user.name+' bucket challenge test'+'.xlsx'
        workbook=xlsxwriter.Workbook(name_of_file)
        table=workbook.add_worksheet()
        table.write(0,0,self.name)
        table.write(0,1,self.id_number)
        #table.write(0,2,self.results)
        i=0
        for result in self.results:
            table.write(1,i,result)
            i=i+1
        table.write(0,3,self.relative_average_result())
        table.write(0,4,self.absolute_average_result())
